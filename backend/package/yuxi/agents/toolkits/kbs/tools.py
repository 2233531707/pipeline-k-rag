...
async def query_kb(
    ...
    如果提供了 graph_entity_ids（来自 query_knowledge_graph 的 retrieval_hints），
    将利用图谱实体作为种子进行 Personalized PageRank 增强检索。
    """
    if not kb_id:
        return "请提供 kb_id"
    if not query_text:
    ...
    if kb_type == "dify" and graph_entity_ids:
        logger.warning("Dify 知识库不支持图谱增强检索，忽略 graph_entity_ids")
    ...
    try:
        ...
        if graph_entity_ids:
            kwargs["graph_entity_ids"] = list(dict.fromkeys(graph_entity_ids))  # 去重保序
...
def get_common_kb_tools() -> list:
    ...
    - query_kb: 在指定知识库中检索
    - query_knowledge_graph: 显式查询知识图谱节点和关系
    - find_kb_document: 在指定文件内定位关键词或正则模式
    - open_kb_document: 按 file_id 分段打开知识库文档
    - list_spatial_layers: 列出空间图层
    - query_spatial_features: 查询空间要素摘要
    - show_spatial_map: 生成会话地图配置
    """
    from yuxi.agents.toolkits.kbs.graph_tools import query_knowledge_graph
    from yuxi.agents.toolkits.kbs.spatial_tools import (
        list_spatial_layers,
