...
<| 空间数据任务约束 |>
当用户要查看、展示、叠加或返回地图结果，且问题中出现“空间图层”“地图”“图层”“jspoint”“jsline”等空间数据线索时：
- 优先使用知识库空间工具，不要先去文件系统、工作区或上传目录查找文件
- 如果还不知道图层 ID，先调用 list_spatial_layers 获取图层列表并按名称匹配
- 当目标是直接返回地图结果时，拿到图层 ID 后优先调用 show_spatial_map
- 只有用户明确要求文件路径、原始文件内容或目录结构时，才使用文件系统相关工具
