...
class BaseContext:
    ...
    knowledge_tools: list[str] | None = field(
        default=None,
        metadata={
            "name": "知识库工具",
            "description": "知识库、知识图谱和空间分析工具。默认启用全部，可按智能体职责选择。",
            "type": "list",
            "kind": "knowledge_tools",
        },
    )
...
_DEFAULT_ALL_CONTEXT_FIELDS = frozenset({"tools", "knowledge_tools", "knowledges", "mcps", "skills"})
...
async def resolve_agent_resource_options(
    ...
    *,
    ...
    if {"tools", "knowledge_tools"} & fields_to_load:
        ...
        if "knowledge_tools" in fields_to_load:
            options["knowledge_tools"] = [
                _resource_option(tool["slug"], tool.get("name"), tool.get("description"))
                for tool in get_tool_metadata(category="knowledge")
                if tool.get("slug")
            ]
