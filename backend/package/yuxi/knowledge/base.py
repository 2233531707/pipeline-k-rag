...
class KnowledgeBase(ABC):
    ...
    async def parse_file(self, kb_id: str, file_id: str, operator_id: str | None = None) -> dict:
        ...
        try:
            ...
            if params.get("source_type") == SOURCE_TYPE_SHAPEFILE_DATASET:
                result = await SpatialDatasetService().import_dataset(
                    kb_id=kb_id,
                    file_id=file_id,
                    source=file_path,
                    source_name=file_meta.get("filename") or file_id,
                    operator_id=operator_id,
                )
                params["source_type"] = SOURCE_TYPE_SHAPEFILE_DATASET
                params["spatial_source_id"] = result.source_id
                params["spatial_summary"] = result.summary
    ...
    async def delete_database(self, kb_id: str) -> dict:
        ...
        if kb_id in self.databases_meta:
            ...
            await SpatialDatasetService().delete_by_db(kb_id=kb_id)
            minio_client = get_minio_client()
            ...
            for file_id, file_info in self.files_meta.items():
                if file_info.get("kb_id") == kb_id:
                    ...
                    db_files[file_id] = {
                        ...
                        "source_type": (file_info.get("processing_params") or {}).get("source_type"),
                        "spatial_source_id": (file_info.get("processing_params") or {}).get("spatial_source_id"),
                        "spatial_summary": (file_info.get("processing_params") or {}).get("spatial_summary"),
                    }
