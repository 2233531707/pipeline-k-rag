...
class MilvusKB(KnowledgeBase):
    ...
    async def restore_chunks(self, kb_id: str, chunks: list[dict[str, Any]], *, batch_size: int = 64) -> int:
        """Restore portable-package chunks and rebuild their Milvus vectors."""
        if not chunks:
            return 0
        if kb_id not in self.databases_meta:
            raise ValueError(f"Database {kb_id} not found")
        ...
        for chunk in chunks:
            ...
            if file_id not in self.files_meta:
                raise ValueError(f"File {file_id} not found for restored chunk")
            normalized = dict(chunk)
            normalized["id"] = normalized.get("id") or normalized["chunk_id"]
            normalized["graph_indexed"] = bool(normalized.get("graph_indexed"))
            chunks_by_file.setdefault(file_id, []).append(normalized)

        restored = 0
        for file_id, file_chunks in chunks_by_file.items():
            for offset in range(0, len(file_chunks), max(1, batch_size)):
                chunk_batch = file_chunks[offset : offset + max(1, batch_size)]
    ...
    async def index_file(
        ...
        try:
            ...
            from yuxi.knowledge.spatial import SOURCE_TYPE_SHAPEFILE_DATASET, SpatialDatasetService

            if params.get("source_type") == SOURCE_TYPE_SHAPEFILE_DATASET:
                await SpatialDatasetService().index_graph_for_file(kb_id=kb_id, file_id=file_id)
    ...
    async def update_content(self, kb_id: str, file_ids: list[str], params: dict | None = None) -> list[dict]:
        ...
        for file_id in file_ids:
            ...
            try:
                ...
                if resolved_params.get("source_type") == SOURCE_TYPE_SHAPEFILE_DATASET:
                    spatial_service = SpatialDatasetService()
                    await spatial_service.delete_by_file(kb_id=kb_id, file_id=file_id)
                    result = await spatial_service.import_dataset(
                        kb_id=kb_id,
                        file_id=file_id,
                        source=file_path,
                        source_name=filename or file_id,
                    )
                    resolved_params["source_type"] = SOURCE_TYPE_SHAPEFILE_DATASET
                    resolved_params["spatial_source_id"] = result.source_id
                    resolved_params["spatial_summary"] = result.summary
                    self.files_meta[file_id]["processing_params"] = resolved_params
                ...
                if resolved_params.get("source_type") == SOURCE_TYPE_SHAPEFILE_DATASET:
                    await spatial_service.index_graph_for_file(kb_id=kb_id, file_id=file_id)
    ...
    async def aquery(self, query_text: str, kb_id: str, agent_call: bool = False, **kwargs) -> list[dict]:
        ...
        try:
            ...
            graph_entity_ids: list[str] | None = merged_kwargs.get("graph_entity_ids")
            if graph_entity_ids and not isinstance(graph_entity_ids, list):
                graph_entity_ids = None
            if use_reranker or use_graph_retrieval or graph_entity_ids:
    ...
    async def _retrieve_graph_chunks(
        ...
        try:
            ...
            # 若提供了显式 graph_entity_ids，直接构建种子权重跳转向量搜索
            if graph_entity_ids:
                seed_weights = {eid: 1.0 for eid in graph_entity_ids}
            else:
    ...
    async def delete_file(self, kb_id: str, file_id: str) -> None:
        ...
        await SpatialDatasetService().delete_by_file(kb_id=kb_id, file_id=file_id)
