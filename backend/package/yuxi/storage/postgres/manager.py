    ...
    async def ensure_knowledge_schema(self):
        ...
        stmts = [
            ...
            CREATE TABLE IF NOT EXISTS knowledge_spatial_sources (
                ...
                original_crs VARCHAR(128),
                layer_count INTEGER DEFAULT 0,
                feature_count INTEGER DEFAULT 0,
                skipped_feature_count INTEGER DEFAULT 0,
                bbox JSONB,
                summary JSONB,
                error_message TEXT,
                created_by VARCHAR(64),
            ...
            """,
            """
            CREATE TABLE IF NOT EXISTS knowledge_spatial_layers (
                id SERIAL PRIMARY KEY,
                layer_id VARCHAR(64) UNIQUE NOT NULL,
                source_id VARCHAR(64) NOT NULL REFERENCES knowledge_spatial_sources(source_id) ON DELETE CASCADE,
                kb_id VARCHAR(80) NOT NULL REFERENCES knowledge_bases(kb_id) ON DELETE CASCADE,
                name VARCHAR(512) NOT NULL,
            ...
            """,
            """
            CREATE TABLE IF NOT EXISTS knowledge_spatial_features (
                id SERIAL PRIMARY KEY,
            ...
            """,
            "ALTER TABLE IF EXISTS knowledge_spatial_features ADD COLUMN IF NOT EXISTS geom geometry(Geometry, 4326)",
            "ALTER TABLE IF EXISTS knowledge_spatial_layers ALTER COLUMN source_id DROP NOT NULL",
            "ALTER TABLE IF EXISTS knowledge_spatial_features ALTER COLUMN source_id DROP NOT NULL",
            "ALTER TABLE IF EXISTS knowledge_spatial_layers ADD COLUMN IF NOT EXISTS created_by VARCHAR(64)",
            """
            CREATE TABLE IF NOT EXISTS knowledge_spatial_layer_compositions (
                id SERIAL PRIMARY KEY,
                composition_id VARCHAR(64) UNIQUE NOT NULL,
                kb_id VARCHAR(80) NOT NULL REFERENCES knowledge_bases(kb_id) ON DELETE CASCADE,
            ...
            CREATE TABLE IF NOT EXISTS knowledge_spatial_layer_composition_items (
                ...
                style_override JSONB NOT NULL DEFAULT CAST('{}' AS JSONB),
                CONSTRAINT uq_spatial_composition_layer UNIQUE (composition_id, layer_id),
                CONSTRAINT uq_spatial_composition_position UNIQUE (composition_id, position)
            )
            """,
            ...
            "CREATE INDEX IF NOT EXISTS idx_kss_kb_id ON knowledge_spatial_sources(kb_id)",
            "CREATE INDEX IF NOT EXISTS idx_kss_file_id ON knowledge_spatial_sources(file_id)",
            "CREATE INDEX IF NOT EXISTS idx_ksl_source_id ON knowledge_spatial_layers(source_id)",
            "CREATE INDEX IF NOT EXISTS idx_ksl_kb_id ON knowledge_spatial_layers(kb_id)",
            "CREATE INDEX IF NOT EXISTS idx_ksf_layer_id ON knowledge_spatial_features(layer_id)",
            "CREATE INDEX IF NOT EXISTS idx_ksf_source_id ON knowledge_spatial_features(source_id)",
