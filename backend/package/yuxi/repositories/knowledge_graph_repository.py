        ...
        async with pg_manager.get_async_session_context() as session:
            result = await session.execute(
                ...
                .where(
                    ...
                    KnowledgeGraphEntity.id > after_id,
                )
        ...
        async with pg_manager.get_async_session_context() as session:
            result = await session.execute(
                ...
                .where(
                    ...
                    KnowledgeGraphTriple.id > after_id,
                )
                .order_by(KnowledgeGraphTriple.id.asc())
                .limit(max(1, limit))
            )
            return list(result.scalars().all())
