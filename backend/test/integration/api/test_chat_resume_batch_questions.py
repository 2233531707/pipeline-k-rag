...
async def _get_agent_id(test_client, headers):
    ...
    agents_response = await test_client.get("/api/agent", headers=headers)
    ...
    if not agent_id:
        pytest.skip("Agent payload missing agent_id/slug field.")
...
async def _create_test_thread(test_client, admin_headers) -> str:
    ...
    return thread_data.get("thread_id") or thread_data.get("id") or thread_id
