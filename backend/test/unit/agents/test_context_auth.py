...
async def test_normalize_agent_context_config_expands_null_and_filters_explicit_lists(monkeypatch):
    ...
    def fake_get_tool_metadata(category=None):
        if category == "knowledge":
            return [{"slug": "query_knowledge_graph", "name": "Graph", "description": ""}]
        return [
    ...
    monkeypatch.setitem(
        ...
        types.SimpleNamespace(get_tool_metadata=fake_get_tool_metadata),
    )
    monkeypatch.setitem(
        sys.modules,
        "yuxi.knowledge",
    ...
    assert normalized["knowledge_tools"] == ["query_knowledge_graph"]
...
async def test_prepare_agent_runtime_context_filters_resources_and_derives_runtime_scope(monkeypatch):
    ...
    def fake_runtime_tool_metadata(category=None):
        if category == "knowledge":
            return [{"slug": "query_knowledge_graph", "name": "Graph", "description": ""}]
        return [{"slug": "ask_user_question", "name": "Ask User", "description": ""}]

    monkeypatch.setitem(
        sys.modules,
        "yuxi.agents.toolkits.service",
        types.SimpleNamespace(get_tool_metadata=fake_runtime_tool_metadata),
    )
    monkeypatch.setitem(
        sys.modules,
        "yuxi.knowledge",
    ...
    context = ChatBotContext(
        ...
        knowledge_tools=["query_knowledge_graph", "missing"],
    ...
    assert prepared.knowledge_tools == ["query_knowledge_graph"]
...
async def test_prepare_agent_runtime_context_clears_resources_for_missing_user(monkeypatch):
    ...
    context = ChatBotContext(
        uid="missing",
        tools=["tool"],
        knowledge_tools=["query_knowledge_graph"],
        knowledges=["kb"],
        mcps=["mcp"],
        skills=["skill"],
        subagents=["agent"],
    ...
    assert prepared.knowledge_tools == []
