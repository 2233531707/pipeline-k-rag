...
async def e2e_agent_context(e2e_client: httpx.AsyncClient, e2e_headers: dict[str, str]) -> dict[str, str | int]:
    ...
    agent_id = agent.get("agent_id") or agent.get("slug")
    if not agent_id:
        pytest.fail("Agent payload missing agent_id/slug field for E2E tests.")
    context = (agent.get("config_json") or {}).get("context") or {}
    return {"agent_id": str(agent_id), "uid": str(uid), "model": str(context.get("model") or "")}


@pytest_asyncio.fixture(scope="function")
async def e2e_llm_ready(
    e2e_client: httpx.AsyncClient,
    e2e_headers: dict[str, str],
    e2e_agent_context: dict[str, str | int],
) -> None:
    model = str(e2e_agent_context.get("model") or "")
    provider_id, separator, _model_id = model.partition(":")
    if not separator or not provider_id:
        pytest.skip("The default E2E agent has no provider-qualified chat model configured.")

    response = await e2e_client.get(f"/api/system/model-providers/{provider_id}", headers=e2e_headers)
    if response.status_code == 403:
        pytest.skip("Checking E2E model credentials requires an admin account.")
    if response.status_code != 200:
        pytest.skip(f"Unable to inspect E2E model provider {provider_id}: {response.text}")
    if (response.json().get("data") or {}).get("credential_status") != "ok":
        pytest.skip(f"Model provider {provider_id} has no configured credentials for live E2E runs.")
