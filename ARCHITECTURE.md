...
地下管网知识模型数据库（Pipeline-K-RAG）是一个面向 RAG、知识图谱、空间数据和多智能体工作流的平台。用户在 Vue 前端中配置智能体、知识库、工具、Skills、MCP 与 SubAgents；前端通过 `/api` 调用 FastAPI；后端服务层协调数据库、对象存储、向量库、图数据库、LangGraph 运行态和沙盒；长耗时智能体运行交给 worker 异步执行，并通过事件流回到前端。
...
5. `worker-dev` 执行 LangGraph 智能体；中间件按上下文挂载知识库工具、Skills、MCP、附件与沙盒能力；知识库工具可在智能体编辑器中独立选择。
...
Milvus 知识库的 chunks、唯一实体和三元组均可建立语义向量，Neo4j 保存实体关系与 chunk 引用。`query_knowledge_graph` 提供显式子图查询；`query_kb(graph_entity_ids=...)` 以实体为种子扩展图谱，并使用 RRF 融合文档与图谱命中。图谱抽取器从知识库 `graph_build_config` 读取独立 Chat 模型 spec。

### 知识库迁移

`backend/package/yuxi/knowledge/migration` 定义 `.yuxikb.zip` 的 manifest、checksum、导入预检、导出和恢复流程。包不携带 API Key、用户 ID 或权限；导入创建新 `kb_id`，恢复业务数据后重建 Milvus 文档/图谱索引和 Neo4j 关系，失败时回滚。

### 空间数据

空间图层和几何数据由 PostgreSQL/PostGIS 管理，原始上传文件由 MinIO 管理。空间 API 位于知识库路由边界，领域实现负责导入、图层组合和派生分析；智能体工具返回结构化空间结果，前端 `SpatialMapResult.vue` 使用 MapLibre 渲染会话地图。
