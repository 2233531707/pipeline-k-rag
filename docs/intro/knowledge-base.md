...
智能体有两条互补链路：`query_knowledge_graph` 用于显式返回实体、关系和子图；`query_kb(graph_entity_ids=...)` 用图谱实体作为检索种子，再将图谱与 chunk 命中通过 RRF 融合。图谱抽取 Chat 模型在知识库创建或图谱配置中单独选择，不与当前对话智能体绑定。

## 迁移与空间数据

`.yuxikb.zip` 可导出原始文件、解析结果、chunks、图谱配置和实体关系。导入时创建新知识库并重建 Milvus、Neo4j 索引。

Milvus 知识库详情中的「空间数据」支持 PostGIS 图层、上传、叠加和派生分析。智能体空间工具可返回地图结果；会话地图支持多图层显隐、顺序、透明度、缩放、Popup 和属性查看。
