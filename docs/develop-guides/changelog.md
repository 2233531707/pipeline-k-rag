...
- 修复 ZIP 安全扫描对 Windows 路径分隔符的误判：共享 ZIP 校验会先将条目中的 `\` 归一化为 `/` 再执行深度、绝对路径和路径穿越校验，避免正常 ZIP 中的 `scripts\audit_uploaded_plan.py` 一类条目被误拒，同时继续拦截 `..\` 与盘符绝对路径。
...
- 完成 Main2 T25 Docker 全量回归：更新集成/E2E 测试到当前 Agent API 契约，验证迁移包导入导出、文档与图谱索引重建、图谱增强检索、PostGIS 空间入库、图层组合和 Agent 知识工具配置链路。
- 新增知识库分组能力：后端持久化全局知识库分组，自动创建真实“默认分组”并为历史知识库补齐默认归属；知识库页面支持新建、重命名、删除分组，创建知识库时选择分组，并可在列表中移动知识库；迁移包导出/导入保留分组名称，智能体可访问知识库接口不暴露分组管理元数据。
- 推翻 Windows 本地单机版交付方向：不再推进后端静态托管前端、`SANDBOX_PROVIDER=local` 本机进程沙盒、native runtime manager 或随客户端安装数据库运行时；桌面交付改为 Electron 前端客户端连接独立后端服务器。
- 首版 Windows 桌面前端客户端实现方向收敛为 Electron：使用 `kb-desktop://app` 自定义协议加载前端，交付 `portable exe`，首版范围只覆盖连接配置、用户名密码登录、独立后端服务器首启初始化和主智能体对话 SSE 主链路。
- 清理 `packaging/windows/` 中与前后端分离桌面客户端方案无关的旧本地单机版残留：删除 `launcher/`、`installer/`、`bundle/`、旧打包测试、旧安装脚本及其构建产物，只保留 Electron 客户端工程、`build_electron_portable.ps1` 和当前 Electron 构建产物目录。
- 补齐桌面前端客户端第二轮对话闭环基础适配：桌面模式下聊天附件预览、图片/图表工具产物和会话地图样式 URL 统一解析到独立后端服务器；切换服务器时广播后端变更并清理对话草稿、未发送附件、本地线程态和运行中 SSE；补充桌面资源 URL 与切换事件前端测试。
- 继续补齐桌面前端客户端第二轮对话产物链路：新增工作区知识库预览、工作区下载与 viewer 文件系统预览/下载的公共 URL builder，并补充线程交付物、工作区文件和 viewer 文件系统桌面下载/预览 URL 回归测试，确保桌面模式下这些入口显式指向独立后端服务器。
- 继续收紧桌面前端客户端第二轮知识库来源预览入口：从回答来源跳转工作区预览时，仅接受 `parsed/original` 两种知识库预览模式，非法 `variant` 自动回退到 `parsed`，避免桌面端 PDF/Markdown 预览入口带入无效模式；补充对应前端回归测试。
- 继续收紧桌面前端客户端第二轮知识库预览模式切换入口：仅当目标模式存在于当前文件的可用预览模式列表中时，前端才发起 `parsed/original` 切换请求，避免桌面端 PDF/图片原文件预览入口被无效模式误触发；补充对应前端回归测试并接入 `pnpm test`。
- 继续补齐桌面前端客户端第二轮附件闭环：抽取待发送附件请求工具，发送消息前统一过滤空 `file_id`、去重生成 `attachment_file_ids`，并只为本次待发送附件写入本地乐观态 `request_id`；补充对应前端回归测试并接入 `pnpm test`。
- 继续补齐桌面前端客户端第二轮临时附件确认链路：抽取附件确认 payload 组装工具，确认时仅提交 `uploaded/parsed` 项，并保留 `parsed_object_name` 与 `truncated` 语义；补充对应前端回归测试并接入 `pnpm test`。
- 继续补齐桌面前端客户端第二轮临时附件解析态链路：抽取默认解析方法、切换解析方法状态回退和解析禁用条件的公共规则，确保切换解析方式时清空旧结果，且在不可用/解析中/确认中状态下不会误触发解析；补充对应前端回归测试并接入 `pnpm test`。
- 继续收紧桌面前端客户端第二轮临时附件解析方法选择：OCR 健康检查返回后，若当前默认方法为空或已变为不可用，则自动切换到首个可用解析方法，避免附件上传后出现“可解析但默认选中项不可用”的桌面端状态；补充对应前端回归测试。
- 继续收紧桌面前端客户端第二轮临时附件弹窗状态边界：关闭附件弹窗时会重置 OCR 健康状态，避免切换服务器或重新打开弹窗后沿用旧服务器的 OCR 可用性结果；补充对应前端回归测试。
- 继续收紧桌面前端客户端第二轮临时附件 OCR 健康检查竞态：旧弹窗会话中发出的健康检查结果仅对当前请求序号生效，关闭后重新打开弹窗时不会被旧请求的晚到响应污染；补充对应前端回归测试。
- 继续收紧桌面前端客户端第二轮临时附件解析方法同步边界：当 OCR 健康检查返回后的可选解析方法集合发生变化，且当前选中方法已不在新集合中时，自动回落到首个可用方法，避免桌面端保留失效旧选项；补充对应前端回归测试。
- 继续收紧桌面前端客户端第二轮临时附件弹窗异步结果会话边界：旧弹窗会话里的上传、解析与确认晚到结果仅对当前弹窗会话生效，关闭后重新打开弹窗时不会再污染新的附件列表、确认状态或提示；补充对应前端回归测试。
- 继续补齐桌面前端客户端第二轮切换服务器状态清理：后端地址变更后，子智能体线程弹窗会同步关闭并清空旧 `child_thread_id/名称/头像`，避免继续显示旧服务器上下文；补充对应前端回归测试。
- 完成桌面前端客户端第二轮代码与打包收尾：重新构建最新 `0.1.0` portable 包并确认最终便携包已更新到白屏修复后的产物，启动后可枚举到主窗口句柄；同时继续复核知识库预览模式、临时附件解析状态、切换服务器清理、附件进入 Run、附件确认 payload 和知识库来源最短跳转链路规则，第二轮剩余事项收敛为围绕最终 `portable exe` 补人工 GUI 验收记录。
- 补充桌面前端客户端第二轮 GUI 验收阻塞记录：当前会话可确认最终 `portable exe` 进程能拉起，但尚未稳定拿到 Electron 前台窗口截图或可见顶层窗口句柄，因此连接页/登录页/主页面等 GUI 链路仍需后续人工逐项补录。
- 修复桌面前端客户端 `exe` 白屏：桌面模式首屏路由新增独立重定向规则，优先处理 `/`、已登录 `/login` 和未配置后端 `/connect`，避免“已登录桌面端命中 `webOnly` 路由后在 `/` 与 `/login` 间死循环”导致首屏白屏；补充对应前端回归测试，并复建最新 `win-unpacked` 验证修复后窗口可见。
...
- 完成 Main2 知识图谱、可迁移知识库与空间分析闭环：新增 `query_knowledge_graph` 显式图谱工具、`.yuxikb.zip` 后台导入导出、GeoJSON/SHP ZIP/GPKG 分批导入、图层组合与派生分析、会话地图工具和管理 UI；参考 UI 模板统一蓝色冷灰设计 token，以“地下管网知识模型数据库”统一产品展示名称，更新产品图标，并重构登录页、首页、应用侧栏、共享页头与工具栏；空间图层预览支持鼠标与触控拖拽、滚轮与双指缩放及视图复位，补齐目标单元、集成和前端测试。
...
- 修复模型返回空工具名时持续重试的问题：首次发现畸形工具调用即终止本次运行并提示切换兼容模型；同时补齐知识图谱与空间工具的元数据注册，使其可在工具管理页查看
- 智能体编辑器新增“知识库工具”配置，可独立选择 RAG、知识图谱和空间分析工具；旧配置默认保持全部知识库工具启用
...
### 修复
- 修复 Electron 桌面前端客户端在 `portable exe` 场景下因 `electron-store` ESM 加载方式和主进程缺失 `fs` 导入导致窗口无法显示的问题
