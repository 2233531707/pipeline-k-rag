  ...
  <a-modal v-model:open="visible" title="添加文件" width="800px" @cancel="handleCancel">
    ...
    <div class="add-files-content">
      ...
      >
        ...
        <div
          class="setting-row"
          v-if="folderTreeData.length > 0 || (uploadMode !== 'url' && !isSpatialUpload)"
          :class="{
            'two-cols': uploadMode !== 'url' && !isSpatialUpload && folderTreeData.length > 0
          }"
        >
          ...
          <div class="col-item" v-if="uploadMode !== 'url' && !isSpatialUpload">
      ...
      <div
        v-if="hasPdfOrImageFiles && !isOcrEnabled && !isSpatialUpload"
      ...
      <div
        class="upload-area"
        v-if="uploadMode === 'file' || uploadMode === 'folder' || uploadMode === 'spatial'"
      >
        <a-upload-dragger
        ...
        >
          ...
          <div class="zip-tip" v-if="isSpatialUpload">
            支持 GeoJSON、SHP ZIP 与 GeoPackage，将导入图层和空间要素
          </div>
          <div class="zip-tip" v-else-if="hasZipFiles">ZIP包将自动解压提取 Markdown 与图片</div>
        </a-upload-dragger>
...
const uploadHint = computed(() => {
  if (isSpatialUpload.value) {
    return '.geojson, .json, .zip（SHP）, .gpkg'
  }
  if (!supportedFileTypes.value.length) {
    return '加载中...'
  }
...
const isSupportedExtension = (fileName) => {
  ...
  if (isSpatialUpload.value) {
    return ['.geojson', '.json', '.zip', '.gpkg'].some((ext) =>
...
const uploadModeOptions = computed(() => [
  ...
  },
  {
    value: 'spatial',
    label: h('div', { class: 'segmented-option' }, [
      h(MapIcon, { size: 16, class: 'option-icon' }),
      h('span', { class: 'option-text' }, '空间数据')
    ])
  },
  {
    value: 'url',
...
const chunkData = async () => {
  ...
  if (uploadMode.value !== 'url' && !isSpatialUpload.value && !validateOcrService()) {
    return
  }
  ...
  if (uploadMode.value === 'workspace') {
    ...
    try {
      ...
      for (const item of importedItems) {
        ...
        const ext = filePath.substring(filePath.lastIndexOf('.')).toLowerCase()
        if (!isSpatialUpload.value && imageExtensions.includes(ext) && !isOcrEnabled.value) {
          message.error({
  ...
  try {
    ...
    const params = { ...processingParams.value, content_hashes, file_sizes }
    if (isSpatialUpload.value) {
      params.source_type = 'shapefile_dataset'
