...
export const databaseApi = {
   ...
   * 导出知识库为便携迁移包
   * @param {string} kbId - 知识库ID
   * @returns {Promise} - 任务信息 { task_id, ... }
   */
  exportPortablePackage: async (kbId) => {
    return apiAdminPost(`/api/knowledge/databases/${kbId}/portable-export`)
  },
   ...
   */
  downloadPortablePackage: async (kbId, taskId) => {
    return apiAdminGet(
      `/api/knowledge/databases/${kbId}/portable-export/${taskId}/download`,
      {},
      'blob'
    )
  },
  ...
  /**
   * 预检迁移包
   * @param {FormData} formData - 包含 .yuxikb.zip 文件
   * @returns {Promise} - 预检报告
   */
  preflightImport: async (formData) => {
    return apiAdminPost('/api/knowledge/portable-import/preflight', formData, {
  ...
  /**
   * 执行迁移包导入
   * @param {Object} importData - { file, target_name, embedding_model_spec, graph_chat_model_spec? }
   * @returns {Promise} - 导入任务 { task_id, ... }
   */
  importPortablePackage: async (importData) => {
    const formData = new FormData()
    formData.append('file', importData.file)
    if (importData.target_name) formData.append('target_name', importData.target_name)
    formData.append('embedding_model_spec', importData.embedding_model_spec)
    if (importData.graph_chat_model_spec) {
      formData.append('graph_chat_model_spec', importData.graph_chat_model_spec)
    }
    return apiAdminPost('/api/knowledge/portable-import', formData, {
      headers: { 'Content-Type': 'multipart/form-data' }
    })
  },

  getPortableImportStatus: async (taskId) => {
    return apiAdminGet(`/api/knowledge/portable-import/${taskId}/status`)
  }
}
...
export const spatialApi = {
  listSources: async (kbId) => {
    return apiAdminGet(`/api/knowledge/databases/${kbId}/spatial/sources`)
  },

  listLayers: async (kbId) => {
    return apiAdminGet(`/api/knowledge/databases/${kbId}/spatial/layers`)
  },

  deleteLayer: async (kbId, layerId) => {
    return apiAdminDelete(`/api/knowledge/databases/${kbId}/spatial/layers/${layerId}`)
  },

  getLayerFeatures: async (kbId, layerId, options = {}) => {
    const params = new URLSearchParams()
    if (Array.isArray(options.bbox)) params.set('bbox', options.bbox.join(','))
    ...
    return apiAdminGet(
      `/api/knowledge/databases/${kbId}/spatial/layers/${layerId}/features${query ? `?${query}` : ''}`
    )
  },

  getFeature: async (kbId, featureId) => {
    return apiAdminGet(`/api/knowledge/databases/${kbId}/spatial/features/${featureId}`)
  },

  listCompositions: async (kbId) => {
    return apiAdminGet(`/api/knowledge/databases/${kbId}/spatial/compositions`)
  },

  createComposition: async (kbId, data) => {
    return apiAdminPost(`/api/knowledge/databases/${kbId}/spatial/compositions`, data)
  },

  updateComposition: async (kbId, compositionId, data) => {
    return apiAdminPut(
      `/api/knowledge/databases/${kbId}/spatial/compositions/${compositionId}`,
      data
    )
  },

  deleteComposition: async (kbId, compositionId) => {
    return apiAdminDelete(
      `/api/knowledge/databases/${kbId}/spatial/compositions/${compositionId}`
    )
  },

  runAnalysis: async (kbId, data) => {
    return apiAdminPost(`/api/knowledge/databases/${kbId}/spatial/analysis`, data)
  }
}
