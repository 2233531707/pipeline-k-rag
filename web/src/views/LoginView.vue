      ...
      <div class="login-card">
        ...
        <div class="card-side is-image">
          ...
          <div class="login-visual-content">
            <span class="visual-eyebrow">AI KNOWLEDGE PLATFORM</span>
            <h2 class="visual-title">
              {{ infoStore.branding?.title || '让知识、图谱与空间数据协同工作' }}
            </h2>
            <p class="visual-description">
              面向复杂业务场景的智能体平台，统一承载知识检索、图谱查询、数据迁移与空间分析。
            </p>
            <div class="visual-feature-list" aria-label="平台能力">
              <span class="visual-feature">RAG 与显式图谱检索</span>
              <span class="visual-feature">可迁移知识库</span>
              <span class="visual-feature">空间图层管理与叠加</span>
              <span class="visual-feature">多用户任务协作</span>
            </div>
          </div>
        </div>
        ...
        <div class="card-side is-form">
          <div class="form-wrapper">
            <div class="form-brand">
              <img v-if="brandLogo" :src="brandLogo" alt="" class="form-brand-logo" />
              <span class="form-brand-name">{{ brandName }}</span>
            </div>
...
const brandName = computed(() => {
  ...
  const brandNameRaw = infoStore.branding?.name?.trim() || '地下管网知识模型数据库'
...
@import '@/assets/css/login-template.less';
</style>
