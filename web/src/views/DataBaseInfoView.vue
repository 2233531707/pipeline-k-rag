  ...
  <div class="database-info-container extension-detail-page">
      ...
      <div class="detail-actions">
        <a-space :size="8">
          <button
            ...
            :disabled="portableExportLoading"
            @click="handlePortableExport"
          >
            <Download :size="14" />
            <span>{{ portableExportLoading ? '导出中' : '导出迁移包' }}</span>
          </button>
          <button
            type="button"
            class="lucide-icon-btn extension-panel-action extension-panel-action-secondary"
    ...
    <div class="database-detail-body">
      ...
      <main class="database-tab-content">
        ...
        <div v-if="isMilvus && activeTab === 'spatial'" class="tab-panel">
          <SpatialDataSection v-if="kbId" :kb-id="kbId" @upload="showSpatialUploadModal" />
        </div>
...
import SpatialDataSection from '@/components/SpatialDataSection.vue'
...
const tabs = computed(() => {
  if (isMilvus.value) {
    return [
      ...
      { key: 'spatial', label: '空间数据', icon: MapIcon },
...
const showSpatialUploadModal = () => {
  showAddFilesModal({ mode: 'spatial' })
}
...
const portableExportLoading = ref(false)
let portableExportToken = 0
const waitForPortableExport = (milliseconds) =>
  new Promise((resolve) => setTimeout(resolve, milliseconds))

const runPortableExport = async () => {
  const token = ++portableExportToken
  portableExportLoading.value = true
  try {
    const queued = await databaseApi.exportPortablePackage(kbId.value)
    if (!queued?.task_id) throw new Error('迁移包导出任务提交失败')

    taskerStore.registerQueuedTask({
      task_id: queued.task_id,
      name: `导出知识库迁移包: ${database.value.name || kbId.value}`,
      task_type: 'knowledge_portable_export',
      payload: { kb_id: kbId.value }
    })

    const deadline = Date.now() + 30 * 60 * 1000
    while (Date.now() < deadline && token === portableExportToken) {
      const response = await taskerApi.fetchTaskDetail(queued.task_id)
      const task = response?.task
      if (task?.status === 'success') break
      if (task?.status === 'failed' || task?.status === 'cancelled') {
        throw new Error(task.error || task.message || '迁移包导出失败')
      }
      await waitForPortableExport(1000)
    }

    if (token !== portableExportToken) return
    if (Date.now() >= deadline) throw new Error('迁移包导出等待超时，请在任务中心查看状态')

    const response = await databaseApi.downloadPortablePackage(kbId.value, queued.task_id)
    ...
    const filename = encodedName
      ...
      : plainName || `${database.value.name || kbId.value}.yuxikb.zip`
    ...
    message.error(error?.message || '迁移包导出失败')
    throw error
  } finally {
    if (token === portableExportToken) portableExportLoading.value = false
  }
}
...
const handlePortableExport = () => {
  Modal.confirm({
    title: '导出知识库迁移包',
    content:
      '导出的迁移包不包含模型 API Key、用户账号和共享权限。目标终端将重新生成向量索引。',
    okText: '开始导出',
    cancelText: '取消',
    onOk: runPortableExport
  })
}
